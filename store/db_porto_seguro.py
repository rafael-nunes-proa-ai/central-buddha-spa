...
def insert_session(session_id: str):
    conversations_db[session_id] = {
        ...
        "messages": [],
        "current_agent": "porto_seguro_agent",
        "context": {
            "session_id": session_id,
            "nome": None,
            "cpf": None,
...
def get_messages_porto_seguro(session_id: str) -> list[ModelMessage]:
    if session_id not in conversations_db:
        insert_session(session_id)
    return conversations_db[session_id]

def add_messages_porto_seguro(session_id: str, new_msgs: list[ModelMessage]):
    if session_id not in conversations_db:
        insert_session(session_id)
...
def delete_session_porto_seguro(session_id):
