...
def validar_horarios_com_duracao(horarios_disponiveis: list, duracao_terapia: int, intervalo_minutos: int = 10) -> list:
    ...
    Lógica:
    - Para cada horário disponível, verifica se há espaço contínuo suficiente
    - Espaço necessário = duracao_terapia + intervalo_minutos
    - Horários são considerados em blocos de 10 minutos
    - A API já filtra horários de pausa do terapeuta automaticamente
    ...
    Exemplo:
        Terapia 60min com intervalo 10min = 70min total (7 blocos):
        - 13:00 → precisa de horários até 14:10 consecutivos
        - 14:00 → precisa de horários até 15:10 consecutivos
    """
    ...
    try:
        ...
        # Calcula quantos blocos de 10 minutos são necessários
        tempo_total_necessario = duracao_terapia + intervalo_minutos
        blocos_necessarios = tempo_total_necessario // 10
        
        print(f"  🔍 VALIDAÇÃO DE HORÁRIOS:")
        print(f"     Duração terapia: {duracao_terapia} min")
        print(f"     Intervalo: {intervalo_minutos} min")
        print(f"     Total necessário: {tempo_total_necessario} min ({blocos_necessarios} blocos de 10 min)")
        
        horarios_validos = []
        
        for i, (horario_str, horario_dt) in enumerate(horarios_dt):
            horario_final_terapia = horario_dt + timedelta(minutes=duracao_terapia)
            
            # Verifica se há blocos consecutivos suficientes a partir deste horário
            blocos_consecutivos = 1  # Conta o horário atual
            horario_atual = horario_dt
            
            # Verifica próximos horários
            for j in range(i + 1, len(horarios_dt)):
                proximo_horario = horarios_dt[j][1]
                diferenca_minutos = (proximo_horario - horario_atual).total_seconds() / 60
                
                # Se o próximo horário está a 10 minutos, é consecutivo
                if diferenca_minutos == 10:
                    blocos_consecutivos += 1
                    horario_atual = proximo_horario
                    
                    # Se já temos blocos suficientes, este horário é válido
                    if blocos_consecutivos >= blocos_necessarios:
                        horarios_validos.append(horario_str)
                        print(f"     ✅ {horario_str} → VÁLIDO (fim {horario_final_terapia.strftime('%H:%M')}, {blocos_consecutivos} blocos)")
                        break
                else:
                    # Encontrou um buraco, este horário não serve
                    print(f"     ❌ {horario_str} → INVÁLIDO (buraco após {blocos_consecutivos} blocos, falta {blocos_necessarios - blocos_consecutivos})")
                    break
            else:
                # Chegou ao fim da lista sem encontrar blocos suficientes
                if blocos_consecutivos < blocos_necessarios:
                    print(f"     ❌ {horario_str} → INVÁLIDO (apenas {blocos_consecutivos}/{blocos_necessarios} blocos até o fim)")
...
async def validar_data_prazo_30_dias(ctx: RunContext[MyDeps], data_texto: str) -> str:
    ...
    resultado = resolver_data(data_texto, agora)
